...
export default function OrdersShow({ order, shippingConfig }) {
    ...
    // Get shipping config (ensure they're numbers)
    const freeThreshold = Number(shippingConfig.free_threshold) || 5000;
    const baseFee = Number(shippingConfig.base_fee) || 50;
    ...
    const handleEmailCustomer = () => {
        toast.success(`Order confirmation email sent to ${order.user?.email}`);
    };

    const CurrentStatusIcon = statusColors[order.status]?.icon || ClockIcon;
    const statusColor = statusColors[order.status];

    // Calculate item subtotal (convert to numbers to avoid concatenation)
    ...
    // Calculate shipping fee dynamically based on subtotal
            ...
            <div className="space-y-6">
                ...
                <div className="rounded-lg bg-white shadow p-6">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center space-x-4">
                            <div className={`rounded-full p-3 ${statusColor?.bg}`}>
                                <CurrentStatusIcon className={`h-6 w-6 ${statusColor?.text}`} />
                            </div>
                            <div>
                                <p className="text-sm text-gray-500">Current Status</p>
                                <p className={`text-xl font-semibold ${statusColor?.text}`}>{order.status}</p>
                            </div>
                        </div>
                    ...
                    <div className="rounded-lg bg-white shadow p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">{new Date(order.user?.created_at).toLocaleDateString()}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-500">Total Orders</p>
                                <p className="text-sm text-gray-600">{order.user?.order_count || 1} orders</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            ...
                            {order.updated_at !== order.created_at && (
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">{new Date(order.updated_at).toLocaleString()}</p>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {order.items.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-500">#{item.product_id}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{item.quantity}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">₱{Number(item.price_snapshot).toLocaleString()}</td>
                                        <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                                            ₱{Number(item.quantity * item.price_snapshot).toLocaleString()}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot className="bg-gray-50">
                                ...
                                <tr>
                                    ...
                                    <td className="px-6 py-4">
                                        {shippingFee > 0 ? (
                                            <span className="text-sm text-gray-600">₱{shippingFee.toLocaleString()}</span>
                                        ) : (
                        ...
                        {order.status !== 'Pending' && (
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Status changed to Paid</p>
                                </div>
                            </div>
                        )}
                        {order.status === 'Shipped' && (
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Status changed to Shipped</p>
                                </div>
                            </div>
                        )}
                        {order.status === 'Completed' && (
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Status changed to Completed</p>
                                </div>
                            </div>
                        )}
                    </div>
