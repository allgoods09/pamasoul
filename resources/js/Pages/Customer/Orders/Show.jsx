...
export default function OrdersShow({ order }) {
    // Calculate subtotal from items (reliable method)
    const subtotal =
        order.items?.reduce((sum, item) => {
            return sum + Number(item.price_snapshot) * Number(item.quantity);
        }, 0) || 0;

    // Use stored total, calculate shipping fee as the difference
    const orderTotal = Number(order.total) || 0;
    const shippingFee = orderTotal - subtotal;
    const total = orderTotal; // Use stored total directly
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
                        <div className="p-6 bg-gradient-to-r from-pamasoul-50 to-white border-b">
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                                ...
                                <div
                                    className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full ${status.bg} ${status.text} self-start`}
                                >
                                ...
                                <div className="space-y-6 relative">
                                    {steps.map((step, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    className={`relative z-10 flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center transition-all ${
                                                        isCompleted
