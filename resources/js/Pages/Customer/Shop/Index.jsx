...
export default function ShopIndex({ products, categories, filters }) {
    ...
    const getHeaderContent = () => {
        if (!currentCategory) {
            return {
                ...
                description: 'Discover our premium collection of fishing tackle',
                icon: '🎣'
            };
        }
        ...
        switch(currentCategory.id) {
            case 1:
                return {
                    title: 'Fishing Rods',
                    description: 'Experience the perfect balance of power and sensitivity. Our rods are crafted for the passionate angler who demands performance. Pull like a king with every cast!',
                    icon: '🎣'
                };
            case 2:
                return {
                    title: 'Fishing Lines',
                    description: 'Strength meets precision. Our premium fishing lines offer superior knot strength, low memory, and incredible durability. Never let the big one get away!',
                    icon: '🎣'
                };
            case 3:
                return {
                    title: 'Fishing Reels',
                    description: 'Smooth, powerful, and reliable. Our reels deliver exceptional drag systems and buttery-smooth retrieves. Dominate the waters with confidence!',
                    icon: '🎣'
                };
            default:
                return {
    ...
    React.useEffect(() => {
        if (searchRef.current && filters?.search) searchRef.current.value = filters.search;
        if (categoryRef.current && filters?.category) categoryRef.current.value = filters.category;
        if (sortRef.current && filters?.sort) sortRef.current.value = filters.sort;
        if (minPriceRef.current && filters?.min_price) minPriceRef.current.value = filters.min_price;
        if (maxPriceRef.current && filters?.max_price) maxPriceRef.current.value = filters.max_price;
    }, [filters]);
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center space-x-3 mb-2">
                            ...
                            <h1 className="text-3xl font-bold text-gray-900">{headerContent.title}</h1>
                        </div>
                        <p className="text-gray-600 mt-2 max-w-2xl">{headerContent.description}</p>
                        {currentCategory && (
                            <div className="mt-3">
